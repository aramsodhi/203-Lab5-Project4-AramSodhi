...
public class WorldParser {
    ...
    private static Entity parseEntity(String parameters, ImageLibrary imageLibrary) {
        ...
        if (args.length >= Entity.ENTITY_PROPERTY_COLUMN_COUNT) {
            ...
            return switch (key) {
                ...
                case Dinosaur.DINOSAUR_KEY -> parseDinosaur(specificProperties, id, position, imageLibrary);
                case MushroomStump.MUSHROOM_STUMP_KEY -> parseMushroomStump(specificProperties, id, position, imageLibrary);
                case Asteroid.ASTEROID_KEY -> parseAsteroid(specificProperties, id, position, imageLibrary);
                case Chicken.CHICKEN_KEY -> parseChicken(specificProperties, id, position, imageLibrary);
    ...
    private static Entity parseDinosaur(String[] properties, String id, Point position, ImageLibrary imageLibrary) {
        if (properties.length == Dinosaur.DINOSAUR_PARSE_PROPERTY_COUNT) {
            return new Dinosaur(
                    id,
                    position,
                    imageLibrary.get(Dinosaur.DINOSAUR_KEY),
                    Double.parseDouble(properties[Dinosaur.DINOSAUR_PARSE_PROPERTY_ANIMATION_PERIOD_INDEX]),
                    Double.parseDouble(properties[Dinosaur.DINOSAUR_PARSE_PROPERTY_BEHAVIOR_PERIOD_INDEX])
            );
        } else {
            throw new IllegalArgumentException(String.format(
                    "%s requires %d properties when parsing, got %d",
                    Dinosaur.DINOSAUR_KEY,
                    Dinosaur.DINOSAUR_PARSE_PROPERTY_COUNT,
                    properties.length
            ));
        }
    }

    private static Entity parseMushroomStump(String[] properties, String id, Point position, ImageLibrary imageLibrary) {
        if (properties.length == MushroomStump.MUSHROOM_STUMP_PARSE_PROPERTY_COUNT) {
            return new MushroomStump(
                    id,
                    position,
                    imageLibrary.get(MushroomStump.MUSHROOM_STUMP_KEY)
            );
        } else {
            throw new IllegalArgumentException(String.format(
                    "%s requires %d properties when parsing, got %d",
                    MushroomStump.MUSHROOM_STUMP_KEY,
                    MushroomStump.MUSHROOM_STUMP_PARSE_PROPERTY_COUNT,
                    properties.length
            ));
        }
    }

    private static Entity parseAsteroid(String[] properties, String id, Point position, ImageLibrary imageLibrary) {
        if (properties.length == Asteroid.ASTEROID_PARSE_PROPERTY_COUNT) {
            return new Asteroid(
                    id,
                    position,
                    imageLibrary.get(Asteroid.ASTEROID_KEY),
                    Double.parseDouble(properties[Asteroid.ASTEROID_PARSE_PROPERTY_ANIMATION_PERIOD_INDEX]),
                    Double.parseDouble(properties[Asteroid.ASTEROID_PARSE_PROPERTY_BEHAVIOR_PERIOD_INDEX])
            );
        } else {
            throw new IllegalArgumentException(String.format(
                    "%s requires %d properties when parsing, got %d",
                    Asteroid.ASTEROID_KEY,
                    Asteroid.ASTEROID_PARSE_PROPERTY_COUNT,
                    properties.length
            ));
        }
    }

    private static Entity parseChicken(String[] properties, String id, Point position, ImageLibrary imageLibrary) {
        if (properties.length == Chicken.CHICKEN_PARSE_PROPERTY_COUNT) {
            return new Chicken(
                    id,
                    position,
                    imageLibrary.get(Chicken.CHICKEN_KEY),
                    Double.parseDouble(properties[Chicken.CHICKEN_PARSE_PROPERTY_ANIMATION_PERIOD_INDEX]),
                    Double.parseDouble(properties[Chicken.CHICKEN_PARSE_PROPERTY_BEHAVIOR_PERIOD_INDEX])
            );
        } else {
            throw new IllegalArgumentException(String.format(
                    "%s requires %d properties when parsing, got %d",
                    Chicken.CHICKEN_KEY,
                    Chicken.CHICKEN_PARSE_PROPERTY_COUNT,
                    properties.length
            ));
        }
    }
